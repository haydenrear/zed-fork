...
use gpui::{
    AppContext, AsyncApp, BorrowAppContext, Context, Entity, EventEmitter, SharedString, Task,
    WeakEntity,
};
...
use language_model::message_handler::{
    AiMessageHandler, MessageHandlerConfig, MessageHandlerRegistry, PostgresDatabaseClient,
    init_message_handler,
};
use language_model::{_retrieve_ids, _retrieve_ids_from, AiMessageContent, create_message_handler};
...
impl AcpThread {
    ...
    pub fn handle_session_update(
        ...
        let title_value = self.title.clone();
        self.ai_message_handler.save_acp(
            &update.clone(),
            &_retrieve_ids_from(&self.session_id.clone(), title_value.or(Some("".to_string().into())).unwrap().to_string()),
        );
    ...
    pub fn send(
        ...
        let title_value = self.title.clone();
        let ids = _retrieve_ids_from(&self.session_id, title_value.or(Some("".to_string().into())).unwrap().to_string());
