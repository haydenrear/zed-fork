...
use language_model::{
    ...
    StopReason, TokenUsage, env_var, get_message_handler_async,
};
...
impl LanguageModel for MistralLanguageModel {
    ...
    > {
        ...
        let ids = _retrieve_ids(&previous_request);
        ...
        async move {
            if let Some(handler) = &message_handler {
                handler
                    .save_completion_req(
                        &previous_request,
                        &ids,
                        LanguageModelArgs::from_request(id.clone(), &previous_request),
                    )
                    .await;
            }
            ...
            Ok(peek_db(
                ...
                message_handler,
                ids,
                LanguageModelArgs::from_request(id, &previous_request),
            ))
        }
        .boxed()
    }
