...
use language_model::message_handler::{
    AiMessageHandler, LanguageModelArgs, TokenRateLimiter, peek_db,
};
use language_model::{
    _retrieve_ids, ApiKeyState, AuthenticateError, EnvVar, IconOrSvg, LanguageModel,
    LanguageModelCompletionError, LanguageModelCompletionEvent, LanguageModelId,
    LanguageModelImage, LanguageModelName, LanguageModelProvider, LanguageModelProviderId,
    LanguageModelProviderName, LanguageModelProviderState, LanguageModelRequest,
    LanguageModelRequestMessage, LanguageModelToolChoice, LanguageModelToolResult,
...
impl LanguageModel for OpenAiLanguageModel {
    ...
    > {
        ...
        let message_handler = cx.update(|cx| get_message_handler_async(cx));
        ...
        if self.model.supports_chat_completions() {
            ...
            async move {
                ...
                Ok(peek_db(
                    mapper.map_stream(completions.await?),
                    message_handler,
                    ids,
                ...
                .inspect(move |event| {
                    if let Ok(event) = event {
                        token_limiter.register_response(event);
                    }
                })
                .boxed())
            }
